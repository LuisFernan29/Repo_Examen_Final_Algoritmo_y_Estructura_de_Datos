...
async function obtenerCantidadRegistros(ruta: string): Promise<number> {
    try {
        const respuesta = await fetch(`https://jsonplaceholder.typicode.com${ruta}`);
        if (!respuesta.ok) throw new Error(`Error al consultar ${ruta}: ${respuesta.statusText} (Código: ${respuesta.status})`);
        
        const datos = await respuesta.json();
        if (!Array.isArray(datos)) throw new Error(`Datos en ${ruta} no son una lista. Tipo de dato: ${typeof datos}`);
...
async function obtenerPostYComentarios(idPost: number) {
    try {
        const respuestaPost = await fetch(`https://jsonplaceholder.typicode.com/posts/${idPost}`);
        if (!respuestaPost.ok) throw new Error(`Error al obtener post ${idPost}: ${respuestaPost.statusText} (Código: ${respuestaPost.status})`);
        ...
        if (!respuestaComentarios.ok) throw new Error(`Error al obtener comentarios del post ${idPost}: ${respuestaComentarios.statusText} (Código: ${respuestaComentarios.status})`);
...
async function obtenerPostsUsuario(idUsuario: number) {
    try {
        const respuesta = await fetch(`https://jsonplaceholder.typicode.com/users/${idUsuario}/posts`);
        if (!respuesta.ok) throw new Error(`Error al obtener posts del usuario ${idUsuario}: ${respuesta.statusText} (Código: ${respuesta.status})`);
...
async function main() {
    ...
    const preguntar = (texto: string): Promise<string> => new Promise(resolve => rl.question(texto, resolve));
